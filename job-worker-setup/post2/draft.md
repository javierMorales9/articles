...
- The section has the right narrative shape: it connects the previous post with the need for shared code before introducing the monorepo.
- The worker explanation should stay high-level here. The detailed Graphile Worker implementation belongs in the next post, so the pseudocode should explain the concept without locking the article into queue-specific details too early.
- The draft needs one concrete Manuscritten example, but it should avoid drifting into the final Graphile enqueue flow. For now, the point is that the worker needs the same domain model, database access, configuration, and logging as the backend.
...
- The section should explain monorepos through the specific refactor, not through a generic definition first.
- The npm workspaces explanation needs to be concrete: workspaces help Node link local packages inside one repository, but the architectural decision is still ours.
- The before/after filesystem examples are useful, but they should stay small enough that the reader sees the dependency boundary rather than getting lost in folder trivia.
...
- The section now needs to move linearly: first move the existing backend into `apps/backend`, then make the root a workspace, then turn the backend into an internal package, then extract `packages/domain`.
- Do not mention worker dependencies or shared packages before the filesystem has a place for them. The reader should see each new folder appear when the refactor needs it.
- The router and domain class examples should appear before the extraction, so the reader understands what code moves and why the imports have to change afterward.
...
- This section should feel like the practical payoff after the refactor: now that the repository has packages, how do installs and commands work?
- Be careful with dependency installation wording. npm workspaces link local packages into the root `node_modules`; dependency hoisting is the normal shape, but version conflicts can still create nested installs.
- The CI examples should stay simple and package-focused. This is not the place to design a full pipeline.
...
- Keep this section short. The goal is only to show that the monorepo is ready to host another runtime.
- Do not explain queue processing yet. That belongs to the next article.
- Mention `apps/worker` as another workspace package, not as a shared package under `packages/`.
