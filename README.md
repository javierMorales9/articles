...
This repository is the home for all blog articles. Each article (or series) lives in its own folder. We usually connect a product/code repo (or worktree) into the article folder via a symlink so Codex can review the actual changes and help write the post based on the real diff.
