...
Ideally, the tests should run in a reasonable time, less than 5 minutes (and ideally in less than a minute) so that you can run them more often.
...
The most important one is that in-memory DBs have **their own implementation**, which covers just **part** of the real DB API, not all of it. Complex features like transactions, constraints, row locks, CTEs, indexes, etc. are generally not supported. And even if they are, who can guarantee that they work the same way as the real database? You can find bugs that occur in a flaky in-memory implementation that do not happen in the DB, and vice versa.
...
In this setup, the production code lives in `src/todos/`, and its tests live in `src/tests/integration/`. But now we care about the Jest setup files, which live under `src/tests/utils/`. Except, of course, for `jest.config.ts`, which is in the root of the app and looks like this:
...
That file is needed because Jest's global setup does not run in the same execution context as the test files. So the only way to share the DB URL with the tests is through a temp JSON file. Each test file will use a function like this in `beforeAll` to extract the content of that file:
...
Note: that `user` relationship will allow an important optimization technique, as we will see later.
...
beforeAll(async () => {
  ...
  // We set up the user that will own the todos for the different tests.
  user = new User({ id: crypto.randomUUID(), name: "Javi" });
  
  // And we initialize the server. We pass the user as a way to indicate that
  // it will be automatically authenticated for all requests. The way you do
  // that will depend on your particular framework and setup.
...
beforeEach(async () => {
  ...
  // previous rows breaking the test.
...
it("lists active todos", async () => {
  ...
  // Now we are trying the endpoint that returns
...
So 2 minutes to run. That number is not shocking for an integration test suite that talks to a DB, but it is still well above the kind of feedback loop I expect from a unit test suite. Unit tests are usually expected to run in something closer to seconds, often under [10 seconds](https://stackoverflow.com/questions/10486/unit-test-execution-speed-how-many-tests-per-second). Two minutes is a different animal during development.

And the worst part is that this number grows with the number of tests.

If one hundred tests take around two minutes, one thousand tests could take something in the region of twenty minutes with the same shape. One thousand tests is not a crazy number in a large codebase. But twenty minutes is impossible to use as a development feedback loop.
...
That turned out to be a great idea. Let's see how it is implemented.

First, an important note: when I say one database per worker, I do not mean one Docker container per worker. I mean one PostgreSQL container with several databases inside it.
...
Then, to create each of the worker databases, we need a way to identify which worker we are running in. For that, Jest gives us `JEST_WORKER_ID`, so the database name can be deterministic:
...
```
And with that, we have everything we need to isolate each worker's test execution in its own database. Now we can increase the Jest workers confidently:
...
The same suite that was taking around two minutes went down to roughly fourteen seconds. Almost a 10x improvement. That's impressive, and we can leave it there, but...

## Can We Improve This Even More?


Going from around 2 minutes to 14 seconds was significant.
...
Ideally, we would like to remove this truncate. But, as we saw in the *Why The Reset Is There* section, deleting the resets could lead to tests failing because of dirty data that previous tests (that ran in the same worker, of course) left there.
...
Most applications already have a natural boundary for this. I am talking, of course, about the user.
...
We can take advantage of this natural distribution to isolate tests from each other if we make each test work with its own fresh user and authenticate as that user.
...
This was the case for our tests. Most of them were company-scoped (we work mainly with B2B). So we could make each test create its own company and avoid cleaning up the database altogether. After removing the unnecessary truncates and regenerating scoped fixtures, the suite went down to around 6 seconds.
...
One PostgreSQL container per server integration run + a database per Jest worker + fresh fixtures for every test (where that is safe).

There are probably more things you can do to reduce the execution time even more. In fact, I've tried a bunch of them. But I haven't found any other improvement that can reduce the time a significant amount (another second, for example).

One idea that I thought could have some impact was to keep the database container alive between test runs. Testcontainers supports reusable containers, so I tried to reuse the container and only rebuild the template database when migrations changed. But the runtime almost did not move. So I reverted it.

There are still optimizations I have not tried that I think could improve efficiency. Especially two:
...
2. And the other one is removing durability settings that do not matter in a throwaway database. Like WAL, or even persisting on disk.

But, for now, the current setup is enough for us. Our current setup, which contains around 300 integration tests, runs in about 10 seconds. Which is more than enough to run them regularly.
