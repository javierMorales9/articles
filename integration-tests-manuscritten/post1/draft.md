# Draft: When I Stopped Mocking the Database

## Introduction

One of the things you first encounter when learning about testing is the testing pyramid. You've probably seen it before:

If you don't remember it, the testing pyramid divides the tests to make in three types.

Unit tests are the base of the pyramid. They are the fastest and cheapest to write. The main idea is that you just test an individual unit. This, of course, is totally ambiguous, and everyone has their own opinion about what a unit is: a method of a class, a service, a controller, etc.
...
Integration tests, in turn, are tests that do integrate with the external services. So these kinds of tests would spawn and forward the queries to a database, for example.

And end-to-end tests do too. The main difference between integration tests and end-to-end tests is that end-to-end tests test it all, while integration tests are limited to one particular integration. So an end-to-end test will test the frontend, API gateway, backend, database, and queue at the same time. An integration test will just test one of these connections, the backend / server and the database, for example.
...
The test pyramid idea is absolutely extended and considered a mantra in todays sofware and, even though the intuition is right and the tests in a well covered sofware tends to follow this distribution, I feel it introduces (at leas for me) a bad way of thinking about what tests to write: The idea that integration tests are just like an improved version of unit tests, and that fulfilling the test pyramid consisted of doing something like this:

1. First, write unit tests for all of whatever you consider units. Let us say, for the case, public methods of your classes.
2. Then take 30% of these unit tests belonging to the most important parts of your app and convert them to integration tests.
3. And take half of those and convert them to end-to-end tests.

And so the natural conclusion one gets is that integration tests come after unit tests because, according to the test pyramid, they are just an improvement of those. And so, instead of writing them from the beginning, you end up saying things like: *for the moment, let us just work on unit tests and we will gradually introduce integration tests over time*. But that time typically never comes.

For years I had this way of thinking about integration tests as just an improvement of unit tests and so, secondary and less important. And for years I comforted myself thinking that *since the important parts of the app are already covered with unit tests I guess we are kind of safe, right?*

But with time I have come to the conclusion that this idea is fundamentally wrong. Integration tests are not second order with regard to unit tests. Unit and integration tests are, essentially, different. It is just not an apples-to-apples comparison.

In fact, integration tests should be used to test things that are **directly impossible** for unit tests to cover. Simply put: if your integration test is testing something your unit tests already cover, then that integration test is useless. And inversely, if the hard part in your code is happening outside your server process or in the interconnection with other services, then you should probably head directly to writing integration tests. Regardless of the percentages in the pyramid. Even if you end up with 70% integration tests.

Of course, that would not be the case most of the time. Most of the time unit tests are able to cover much of your code. And integration tests will be less necessary. So you will end up fulfilling the test pyramid and writing 80% of unit tests. 

But there will also be regions of your code were not much is happening inside your server. Were your code is just a wrapper around an external service or database. And in those cases, writing a 80% of unit tests asserting that your mock returns the value you set up to return is kind of useless. You'll get a much better return of investment if you just spawn the external server and write integration tests for it. 

In this article I will try to unpack the typical cases where I find writing integration tests makes more sense. And in the next article I will share with you the simplest way I have found to create this kind of test in a Node.js app using testcontainers.

But first, to have something to compare to, let us see the kind of situation where unit tests shine.

## Section 1: Where unit tests shine
...
So, in an ecommerce app, domain logic is the one that decides whether a product can be added to a cart, how discounts combine, when an order can be cancelled, or how much stock remains after a purchase.
...
But the user will judge the software by whether the business behavior works. What matters to them is whether the cart reserve the product correctly, the accounting system calculate the balance correctly and the banking app prevent an invalid transfer.
...
The idea is to isolate that domain code from infrastructure concerns: database access, HTTP frameworks, queues, external APIs, file systems, etc. so that if we want to change some of it later (change from one framework to another) the domain logic doesn't have to be touched.
...
async function changeUserNameController(req: Request, res: Response) {
  const userRepository = new PostgreRepository(db);

  const previousName = await changeUserName(
    req.params.userId,
    req.body.name,
    userRepository,
  );
...
There two reasons why this make sense:
...
One important point about the repository is implemented is that it works with domain classes directly. It returns a `User` object and receives a `User` object as a parameter.

The implementation just extracts the data and constructs the object when reading, and gets the data from the object and performs the update when saving. In principle, this repositories should not contain any domain logic. They should just work as a wrapper around the database, as our example suggest. Later on, we will see that this condition is not always met, and there are instances where domain logic ends up finding its way into the repository.

## Section 2: Why This Is Great for Unit Tests
...
it("rejects names that are too short", async () => {
  ...
  await expect(
    changeUserName("user-1", "abc", repo),
...
In a general case, you should try to apply this way of working, and encapsulate all the domain logic inside your domain classes. But unfortunately...

## Section 3: Sometimes, the domain logic lives outside your classes
...
To avoid extending this article too much, in this article, I'm going to focus on cases where the domain logic end up living in the database queries (and by extension, the repository).

When dealing with databases, the main reason why this situation happens is because some problems are naturally set-based, transactional, or concurrency-sensitive such that, if we forced all the logic to live the domain layer, we would end up doing something much worse: loading a ridiculous amount of data into memory, reimplementing database behavior in application code, or making a concurrent operation unsafe just so the repository can remain "pure."

Sometimes moving logic into SQL gives us the simpler and faster design and much better performance. And it make sense to do the tradeoff.

To illustrate the point, let's now see two real world examples I have found where the logic ended up living inside database queries:

1. A query that calculates active users for a dashboard.
2. A mutation that reserves ecommerce stock atomically.

## Section 4: Query Example: Active Users Dashboard
...
Of course, doing it this way makes no sense given the fact that the database already knows how to group, count, join, and filter large sets of rows. Pulling all of that into the server just so the we can preserve the logic in the domain layer makes no actual sense.
...
Unit tests cannot help us wit this kind of situations. The only way to test it would be to spanw a database, fill it with test data and check that it returns what we expect for the input data. So, basically, an integration test.

## Section 5: Mutation Example: Reserving Ecommerce Stock
...
This version splits the check and the update into two separate steps, what is known as a read-modify-write cycle. With one request, this code works perfectly fine. But it breaks under concurrency. With two concurrent requests, it can reserve more stock than exists. Suppose the initial state is:
...
PostgreSQL can help you solve this issue by expressing the whole reservation as one update:
...
This query decrements the stock only if the current stock is still greater than zero. If another request already took the last unit, the update returns no rows and that way we know the reservation didn't happen.
